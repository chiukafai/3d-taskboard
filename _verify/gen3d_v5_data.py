# -*- coding: utf-8 -*-
"""v5 平面图 → 3D 看板数据（ZONES / 公共区地板 / 公共区外墙段）

思路（与 floorplan-svg-generator 技能的栅格法一致）：
  1) 直接复用 plan_v5.json 的房间矩形与公共区矩形，1:1 当作 three.js 世界坐标
...
PLAN = json.load(open(os.path.join(HERE, "plan_v5.json"), encoding="utf-8"))
G = 0.1
EX, EZ = PLAN["envelope"]
...
json.dump(dict(zones=zones, pub_floors=pub_floors, pub_walls=pub_walls, pad_rects=pad_rects,
          ...
          open(os.path.join(HERE, "gen3d_v5.json"), "w", encoding="utf-8"),
          ensure_ascii=False, indent=1)
print("\n→ 已写出 _verify/gen3d_v5.json")
